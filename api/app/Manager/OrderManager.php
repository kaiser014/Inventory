...
class OrderManager{
    ...
    private const ORDER_PREFIX = 'DAK';

    public static function generateOrderNumber(int $id){
        return self::ORDER_PREFIX.$id.Carbon::now()->format('dmy').random_int(100,999);
    }
        ...
        if(isset ($input['carts'])){
            foreach($input['carts'] as $key => $cart){
                ...
                if($product && $product->stock >= $cart['quantity']){
                    ...
                    return [
                        'error_description' => $product->name.' Product Stock Out Or Not Exists'
                    ];
                    break;
                }
            }
